...
public class Model extends Observable implements Receiver,Cloneable {
    ...
    private boolean normal(Message message) {
        ...
        if(history!=null) {
            if(n>0) history.model.failure(n+"other buttons are out of sync before");
            else history.model.success();
            if(n>0) l.warning("#"+history.model.attempts()+", "+n+" other buttons are out of sync before using: "+message.string+"!="+toCharacters());
        } else {
            l.warning("history is null! &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&");
            if(n>0) l.warning(n+" other buttons are out of sync before using: "+message.string+"!="+toCharacters());
        }
