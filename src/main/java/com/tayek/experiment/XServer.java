...
public class XServer implements Runnable {
    // maybe we do not need a new server here
    // mayne we just need a server2 impl that never addss connections?
    // this could start with all the servers listening
    // we will get new connections,
    // but we will know ?
    ...
    @Override public void run() {
        l.info(id()+" accepting on: "+serverSocket);
        while(!isShuttingDown) {
            try {
                ...
                if(isShuttingDown) {
                    l.info(id()+" server is shutting down");
